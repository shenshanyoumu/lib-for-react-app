...
import { getUIReducers, getUIReducersImmer, getUIActions } from './moduleUI';
...
export default ({ namespace, state, reducers, actions = {}, thunks = {} }) => {
  ...
  const isImmutable = Immutable.Iterable.isIterable(state);
  const needInjectUI = isImmutable
    ? state.has('ui')
    : Object.prototype.hasOwnProperty.call(state, 'ui');
  ...
  if (needInjectUI) {
    UIReducers = isImmutable
      ? getUIReducers(namespace)
      : getUIReducersImmer(namespace);
